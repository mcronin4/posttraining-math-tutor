"""
On-policy distillation with Socratic tutoring environment.

This module provides the core distillation logic including dataset builders
and configuration for training tutor models using Socratic tutoring environments.
"""
...
# Student profiles based on MathDial dataset and additional variations
# These profiles are general enough to apply to any math problem type
STUDENT_PROFILES = [
    "Steven is a 7th grade student. He has difficulty determining which pieces of information are relevant and which are irrelevant to solving the problem.",
    "DeAndre is a 7th grade student. He has problem with understanding of what steps or procedures are required to solve a problem.",
    "Winnie is a 7th grade student. She struggle to put the numbers in the correct order in the equation or determine the correct operation to use.",
    "Michael is a 7th grade student. He struggle to recognize the problem type and therefore do not know what strategy to use to solve it.",
    "Winnie is a 7th grade student. She has problem with understanding of underlying ideas and principles and a recognition of when to apply them.",
    # Additional profiles for diversity (general enough for any problem type)
    "Alex is a 7th grade student. They often rush through problems and make calculation errors because they don't check their work.",
    "Sam is a 7th grade student. They get confused when problems have multiple steps and sometimes forget what they were trying to find.",
    "Casey is a 7th grade student. They have difficulty understanding when to use different mathematical operations and often choose the wrong one.",
    "Riley is a 7th grade student. They sometimes misunderstand what the problem is asking and solve for the wrong quantity.",
    "Morgan is a 7th grade student. They have trouble organizing their work and lose track of intermediate steps in multi-step problems.",
]


def get_random_student_profile() -> str:
    """Return a random student profile from the available profiles."""
    return random.choice(STUDENT_PROFILES)
...
class SocraticDatasetBuilder(RLDatasetBuilder):
    ...
    async def __call__(self) -> tuple[SocraticDataset, SocraticDataset | None]:
        ...
        # Each seed gets a random student profile for diversity
        def env_group_builder_factory(seed: SeedContext) -> SocraticEnvGroupBuilder:
            # Select a random profile for this seed
            random_profile = get_random_student_profile()
            return create_socratic_env_group_builder(
                ...
                student_profile=random_profile,
...
def build_socratic_config(
    ...
    **kwargs,
    ...
    Note: Each seed/environment gets a randomly selected student profile from a predefined
    list of profiles based on MathDial dataset examples. This provides diversity in student
...
def build_offpolicy_socratic_config(
    tutor_model_name: str,
    teacher_model_name: str,
    learning_rate: float,
    max_tokens: int,
    log_path: str,
    groups_per_batch: int = 1,
    conversations_file: Optional[Path] = None,
    ...
    **kwargs,
    ...
    """
    Build a Config for off-policy distillation using pre-existing conversation trajectories.
    
    This function loads conversation trajectories from MathDial and SocraticMATH datasets
    that have been processed into the unified format. The environment replays these
    conversations, creating training examples from each tutor response with full
    conversation history.
    
    For SocraticMATH: The first teacher message is skipped as a training target but
    included in the conversation history for all subsequent training examples.
    ...
    Args:
        ...
        log_path: Path for logging and checkpoints
        groups_per_batch: Number of environment groups per batch
        conversations_file: Path to single JSONL file with conversation trajectories
        conversations_limit: Optional limit on number of conversations to load (when using conversations_file)
        conversations_files: List of (file_path, limit) tuples for sequential loading.
                           Example: [(mathdial_file, 2000), (socraticmath_file, 3000)]
                           Loads limit from each file sequentially.
        base_url: Optional base URL for Tinker service (defaults to None for local)
        **kwargs: Additional config parameters
        
    Returns:
        Config object configured for off-policy Socratic tutoring distillation with
        kl_penalty_coef=1.0 and lora_rank=32
    """
    from training.distillation.offpolicy_dataset import OffPolicySocraticDatasetBuilder
    
    # Validate arguments
    if conversations_file is None and conversations_files is None:
        raise ValueError("Must provide either conversations_file or conversations_files")
    if conversations_file is not None and conversations_files is not None:
        raise ValueError("Cannot provide both conversations_file and conversations_files")
    
    # Create dataset config
    dataset_config = DistillationDatasetConfig(
        dataset_builder=OffPolicySocraticDatasetBuilder(
            conversations_file=conversations_file or Path(""),  # Dummy if using conversations_files
            tutor_model_name=tutor_model_name,
            groups_per_batch=groups_per_batch,
